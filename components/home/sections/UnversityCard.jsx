...
const UniversityCard = ({
  ...
  const handleNavigate = () => {
    router.push(`/pages/home/universities/${slug}`);
  };
  ...
  return (
    <div className="bg-white rounded-[6px] p-3.5 drop-shadow-sm hover:shadow-md transition-shadow duration-200">
      ...
      {/* Content */}
      <p className="text-xs text-start text-primaryheading font-medium font-inter mb-2">
        {locations}
      </p>
      <hr className="my-2 border-[#D8D8D8] " />
      <p className="text-sm text-start text-primaryheading font-medium font-inter  mb-2">
        Tuition {tuition}
      </p>
      <p className="text-xs text-start text-Primarytext font-medium font-inter mb-1">
        Application fee {applicationFee}
      </p>
      <p className="text-xs text-start text-Primarytext font-medium font-inter mb-3">
        Deposit {deposit}
      </p>
